...
export function StatusBadge({ status, className, showDot = true }: StatusBadgeProps) {
  const normalizedStatus = status as StatusVariant;

  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border",
        statusStyles[normalizedStatus] ?? "bg-zinc-500/10 text-zinc-400 border-zinc-500/20",
        className
      )}
    >
      {showDot && (
        <span
          className={cn(
            ...
            dotColors[normalizedStatus] ?? "bg-zinc-400"
          )}
        />
      )}
      {status}
