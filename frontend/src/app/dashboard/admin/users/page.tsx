...
export default function UsersPage() {
  useTitle("User Management", "Manage all creators and earners on the platform.");
  const [search, setSearch] = useState("");
  const [activeTab, setActiveTab] = useState<"all" | "creator" | "earner">("all");
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="opacity-0 h-0 overflow-hidden absolute">
            ...
            <p className="text-zinc-400 text-sm">Manage all creators and earners on the platform.</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="flex p-1 bg-white/[0.02] border border-white/[0.08] rounded-xl">
            {(["all", "creator", "earner"] as const).map((tab) => (
              <button
                key={tab}
